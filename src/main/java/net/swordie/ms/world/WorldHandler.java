    ...
    private static void handleAttack(Client c, AttackInfo attackInfo) {
        ...
        boolean noCoolTimeAttackHeader =
                attackInfo.attackHeader == OutHeader.SUMMONED_ATTACK ||     // Summon Attack
                        attackInfo.inHeader == InHeader.USER_AREA_DOT_ATTACK;       // Affected Area Attack
        ...
        if (SkillConstants.isDelayedCooldownSkill(skillID) ||  attackInfo.byUnreliableMemory || noCoolTimeAttackHeader || multiAttack || chr.hasSkillCDBypass() || (SkillConstants.isKeyDownSkill(skillID) || chr.checkAndSetSkillCooltime(skillID)) || extraSkills) {
            ...
            if (si != null && si.getExtraSkillInfo().size() > 0) {
                ...
                chr.write(CField.registerExtraSkill(chr, skillID, extraSkillList));
            }
            ...
            if (faSkill > 0) {
                chr.write(CField.finalAttackRequest(chr, skillID, faSkill));
            }
            ...
            for (MobAttackInfo mai : attackInfo.mobAttackInfo) {
                ...
                if (mob == null) {
                    chr.chatMessage(Expedition, String.format("Wrong attack info parse (probably)! SkillID = %d, Mob ID = %d", skillID, mai.mobId));
                    // Lol :p
                } else if (mob.getHp() > 0) {
                    long totalDamage = 0;
...
//                trunk.getItems().sort(Comparator.comparingInt(Item::getItemId));
    ...
    public static void handleUserMigrateToCashShopRequest(Client c, InPacket inPacket) {
        ...
        chr.write(WvsContext.broadcastMsg(BroadcastMsg.popUpMessage(String.format("This feature is currently disabled, add Poki#6969 on discord if you want a fully working higher version source."))));
        chr.dispose();
        return;
    ...
    public static void handleRuneStoneUseRequest(Client c, InPacket inPacket) {
        ...
        if (!chr.hasSkillOnCooldown(runestoneSkillId)) {
            ...
            if (minLevel > chr.getStat(Stat.level)) {
                chr.write(CField.runeStoneUseAck(5));
                return;
            }

            // Send Arrow Message
            chr.write(CField.runeStoneUseAck(8));
        } else {
    ...
    public static void handleRuneStoneSkillRequest(Client c, InPacket inPacket) {
        ...
        chr.addSkillCoolTime(80002280, GameConstants.RUNE_COOLDOWN_TIME * 60 * 1000);
        chr.dispose();
    }
    ...
    public static void handleAttack(Char chr, InPacket inPacket, InHeader header) {
        ...
        if (header != InHeader.USER_MAGIC_ATTACK && header != InHeader.USER_NON_TARGET_FORCE_ATOM_ATTACK && header != InHeader.USER_AREA_DOT_ATTACK) {
...
//        chr.write(UserLocal.setDressChanged(on, true)); // causes client to send this packet again
    ...
    public static void handleShootObjectExplodeRequest(Char chr, InPacket inPacket) {
        ...
        if (JobConstants.isFirePoison(chr.getJob()) && chr.hasSkill(400021028)) {
            ((Magician) chr.getJobHandler()).setExplodeShootObjList(shootObjectIdList);
            EventManager.addEvent(() -> ((Magician) chr.getJobHandler()).getExplodeShootObjList().clear(), 6, TimeUnit.SECONDS);
        }
    }
